...
export function HistoricalReportView() {
	...
	const filteredData = mockHistoricalData.filter((record) => {
		const matchesSearch =
			!searchTerm ||
		...
		const matchesStatus = statusFilter === "Sin Especificar" || record.estadoAcademico === statusFilter;
		...
		return (
			<div className="bg-white px-3 sm:px-6 lg:px-[45px] rounded-lg shadow-lg border border-gray-200">
									...
									{["Sin Especificar", "Activo", "Inactivo"].map((status) => (
										<button
											...
											className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${
												status === statusFilter ? "bg-blue-50 text-blue-700" : "text-gray-700"
				...
				<div className="px-3 py-4">
					<HistoricalTable records={paginatedData} />
				</div>
							...
							return (
								<button
									...
									className={`px-1 py-1 text-xs rounded-full min-w-[30px] min-h-[30px] border border-gray-300 font-bold ${
										currentPage === pageNumber ? "bg-[#2E4258] text-white" : "bg-white hover:bg-gray-100"
									}`}
								>
									{pageNumber}
								</button>
	...
	if (showCombinedView) {
		return (
			<div className="bg-white px-3 sm:px-6 lg:px-[45px] rounded-lg shadow-lg border border-gray-200">
				...
				<div className="flex gap-6 p-6">
					...
					<div className="w-1/3 flex flex-col justify-center">
						<div
							...
							onDrop={handleDrop}
						>
							<svg className="w-12 h-12 mx-auto mb-4 text-teal-500" fill="currentColor" viewBox="0 0 79 113">
							...
							<div className="space-y-2">
								...
								<p className="text-sm text-gray-600">o haz click para seleccionarlo</p>
								<p className="text-xs text-gray-500">Formatos permitidos: ? (máx. ?MB)</p>
							</div>
							<input
								type="file"
								className="hidden"
							...
							<div className="divide-y divide-gray-200">
								{[
									...
									<div key={index} className="grid grid-cols-4 gap-4 px-4 py-3 text-center text-sm">
										...
										<div className="text-gray-900">{item.filename}</div>
										<div>
											<span
											...
											>
												{item.status === "valid" ? "Válido" : "Rechazado"}
											</span>
										</div>
									</div>
								))}
				...
				<div className="flex justify-between items-center px-3 pt-4 sm:pt-6 pb-4 border-t-2 border-[#16469B]">
			...
			{showUploadModal && (
				<Modal isOpen={showUploadModal} onClose={handleCloseModal}>
					<div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-xl p-6">
						<h2 className="text-lg font-semibold mb-4 text-gray-800">Cargar Archivo</h2>
							...
							>
								{uploadedFile ? (
									...
									<>
										...
										<p className="text-sm text-gray-600">Arrastra tu archivo aquí o</p>
										<input
											type="file"
											className="hidden"
											accept=".csv,.xlsx,.xls"
